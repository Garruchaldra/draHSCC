...
class Lister extends Component {
	...
	public function find_sub_components($requested_component, $vce, $components, $sub_components) {
		...
		$query = "SELECT * FROM " . TABLE_PREFIX . "components AS a JOIN " . TABLE_PREFIX . "components_meta AS b ON a.component_id=b.component_id WHERE b.meta_key='title' AND a.component_id IN (SELECT a.component_id FROM " . TABLE_PREFIX . "components_meta AS a JOIN " . TABLE_PREFIX . "components AS b ON a.component_id=b.component_id WHERE a.meta_key='members' AND a.meta_value LIKE '" . $search . "' AND b.parent_id='" . $requested_component->component_id . "') ORDER BY a.sequence";
			...
			if (count($access_data) == 1) {
				...
				foreach ($access_data as $each_access_data) {
					...
					if (!empty($each_access_data->url)) {
						$title = isset($each_access_data->meta_value) ? $each_access_data->meta_value : 'Link';
						...
						$vce->content->add('main','<div class="' . $class . '"><a href="' . $vce->site->site_url . '/' . $each_access_data->url . '" title="' . $title . '">' . $title . '</a></div>'  . PHP_EOL);
					}
	...
	public function as_content($each_component, $vce) {
		...
		if (in_array($vce->user->role_id, explode('|', $each_component->recipe['lister_authority']))) {
			...
			$vce->site->add_style(dirname(__FILE__) . '/css/style.css','coaching-partnership-style');
...
EOF;
	...
	public function update_members($input) {
		...
		if (!empty($input['user_ids']) && !empty($input['component_id'])) {
