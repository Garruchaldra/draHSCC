	...
	public static function file_upload_method($requested_url, $vce) {
		...
		if ((!defined('MEDIA_UPLOAD_PATH') && strpos($requested_url, 'upload') !== false && strlen($requested_url) == 6) || (defined('MEDIA_UPLOAD_PATH') && strpos($requested_url, MEDIA_UPLOAD_PATH) !== false) && strlen($requested_url) == strlen(MEDIA_UPLOAD_PATH)) {
			...
			if (!$chunks || $last_chunk) {
				...
				/**
				 * Emergency Patch
				 * Type and Procedure used to be passed through $_REQUEST, so they were available during all calls to Upload.
				 * The bug being patched is that the $dossier variable was only being created in the first chunk, or if there is no chunking.
				 * $dossier was then recreated below, but without the values from $_REQUEST['dossier']
				 * Dayton Allemann, May 25th, 2022
				 */
				if (!isset($dossier)) {
					$dossier = json_decode($vce->user->decryption($_REQUEST['dossier'], $vce->user->session_vector));
				}
...
EOF;
		...
		$upload_file .= $vce->content->create_input($input,Upload::language('Title'),'Enter a Title');
