...
EOF;
            ...
            $accordion .= self::cycle_though_recipe($recipe);
...
EOF;
    ...
    private function cycle_though_recipe($current_recipe, $component_id = null, $previous_exist = true, $level = 0) {
        ...
        if (isset($component_id)) {
            ...
            //$query = "SELECT * FROM " . TABLE_PREFIX . "components AS a LEFT JOIN " . TABLE_PREFIX . "components_meta AS b ON a.component_id=b.component_id WHERE a.parent_id='" . $component_id . "' AND b.meta_key='type' ORDER BY a.sequence ASC";
            $query = "SELECT * FROM " . TABLE_PREFIX . "components AS a LEFT JOIN " . TABLE_PREFIX . "components_meta AS b ON a.component_id=b.component_id WHERE a.parent_id='" . $component_id . "' ORDER BY a.sequence ASC";

            $requested_component_data = $vce->db->get_data_object($query, false);
            
			foreach ($requested_component_data as $meta_data) {
		
				if (!isset($current_level[$meta_data['component_id']])) {
					// create object and add component table data
					$current_level[$meta_data['component_id']] = array();
					$current_level[$meta_data['component_id']]['component_id'] = $meta_data['component_id'];
					$current_level[$meta_data['component_id']]['parent_id'] = $meta_data['parent_id'];
					$current_level[$meta_data['component_id']]['sequence'] = $meta_data['sequence'];

					// found a url so make sub_url = true
					if (!empty($meta_data['url'])) {
						$current_level[$meta_data['component_id']]['url'] = $meta_data['url'];
					}
				...
				// add meta_value
				$current_level[$meta_data['component_id']][$key] = (($key != 'recipe') ? $vce->db->clean($meta_data['meta_value']) : $meta_data['meta_value']);
			
				// adding minutia if it exists within database table
				if (!empty($meta_data['minutia'])) {
					$key .= "_minutia";
					$current_level[$meta_data['component_id']][$key] = $meta_data['minutia'];
				}
        ...
        foreach ($current_recipe as $counter=>$each_item) {
            ...
            if (isset($each_item['auto_create']) && $each_item['auto_create'] == "forward") {
        		...
        		if (!empty($current_level)) {
        			...
        			foreach ($current_level as $current_level_item) {
        			
        			 	// $current_level_item->meta_value is type
						if ($current_level_item['type'] == $each_item['type']) {
						
							$each_item['component_id'] = $current_level_item['component_id'];
							$each_item['created_at'] = $current_level_item['created_at'];
							$each_item['created_by'] = $current_level_item['created_by'];
                ...
                if (isset($each_item['component_id'])) {
                    ...
                    if (isset($current_level) && count($current_level)) {
            ...
            if (!$check) {
            	...
            	if (!empty($current_level)) {
					...
					'created_by' => $each_item['created_by']
					);
