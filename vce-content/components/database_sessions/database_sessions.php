...
class DatabaseSessions extends Component {
	...
	private static function javascript_timer($expires, $vce) {
		...
		$message = DatabaseSessions::language('Your session has expired');
		$login = DatabaseSessions::language('Click here to login');
	...
	var count = setInterval(
		function() {
			...
			if (current.getTime() > expires) {
				...
				$div.html("<span>' . $message . '<br><br><a class=\"link-button\" href=\"\">' . $login . '</a></span>");
			}
		}, 1000
	);
});
