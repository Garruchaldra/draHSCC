...
class Pbc_resource_library extends Component {

	/**
	 * basic info about the component
	 * Please see the file ./pbc_resource_library_description.txt for more information.
	 */
	public function component_info() {
		return array(
			'name' => 'Pbc Resource Library',
			'description' => 'Custom Resource Library for Materials, customized for OHSCC',
			'category' => 'state',
			'permissions' => array(
				array(
	...
	public function installed() {
		...
		global $vce;
		// don't create a datalist for the taxonomy here, because that is now a recipe instance dependent datalist (there can be multiple taxonomies)

		// $attributes = array (
		// 'datalist' => 'pbc_resource_library_taxonomy',
		// 'aspects' => array ('name' => 'Pbc Resource Library Taxonomy'),
		// );
	...
	public function preload_component() {
			...
			$content_hook = array (
			'media_file_uploader' => 'Pbc_resource_library::media_file_uploader',
			'mediatype_add' => 'Pbc_resource_library::media_file_uploader'
			);
	...
	public static function media_file_uploader($recipe_component, $vce) {
		...
		// get the parent component of this add media call
		                // look for site on parent
						$parent = end($vce->page->components);
						while($parent->type != 'Pbc_resource_library') {
								if (isset($parent->parent)) {
										$parent = $parent->parent;
								} else {
										// there's a problem so return false
										return false;
								}
						}
		// $parent_of_media =isset($vce->page->components) ? end($vce->page->components) : null;

		if (isset($parent->type) && $parent->type == "Pbc_resource_library") {
			$parent_component_id = $parent->component_id;

$content_media = <<<EOF
<input type="hidden" id="resource-library-component-id" class="resource-library-component-id" name="resource_library_component_id" value="$parent_component_id "></input>
<label>
...
EOF;
			...
			if (!isset($vce->content_media)) {
				$vce->content_media = self::taxonomy_menus(array(), $vce->assigned_taxonomy_component_id);
	 ...
	 */
	private static function taxonomy_menus($requested_id = array(), $this_component_id = NULL) {
		if (is_null($this_component_id)) {
			return false;
		}
		global $vce;
		...
		$taxonomy = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', 'component_id' => $this_component_id));
...
// $vce->dump($each_component->component_id);
// $vce->dump($vce->search_results);
// return false;
		...
		/*
		There can be multiple instances of the resource library.
		Each instance (recipe) of the RL can have one taxonomy.
		In the recipe, it is possible to choose which taxonomy to use (meaning NOT the taxonomy associated with the recipe component id you are in).
		$assigned_taxonomy_component_id is the component id associated with the datalist from the recipe component used for this taxonomy.
		It is not possible to edit the chosen $assigned_taxonomy_component_id unless you are in that recipe
		*/
		if ((isset($each_component->recipe['assigned_taxonomy']) && !empty($each_component->recipe['assigned_taxonomy']))) {
			// $vce->dump($each_component->recipe['assigned_taxonomy']);
			$assigned_taxonomy_component_id = $each_component->recipe['assigned_taxonomy'];
		} else {
			$assigned_taxonomy_component_id = $each_component->component_id;
		}
		// assign to $vce
		$vce->site->add_attributes('assigned_taxonomy_component_id', $assigned_taxonomy_component_id, TRUE);
		// $vce->dump($vce->assigned_taxonomy_component_id);
		...
		// get datalist that is associated with this component
		$taxonomy = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', 'component_id' => $vce->assigned_taxonomy_component_id));
		...
		if (empty($taxonomy)) {
			...
			$attributes = array (
			'datalist' => 'pbc_resource_library_taxonomy',
			'aspects' => array ('name' => $each_component->url . ' Pbc Resource Library Taxonomy'),
...
// 		$content .= $categoryAccordion;
		...
		'resource_library_component_id' => $each_component->component_id,
		'this_url' => $this_url
		);
		// generate dossier
...
EOF;
		...
		$dossier = array(
			...
			'this_component_id' => $vce->assigned_taxonomy_component_id,
			'this_url' => $each_component->url
			);
...
EOF;
	 ...
	 */
	public function slow_content_load_resource_library($input) {
		global $vce;

		$test_for_taxonomy_datalist = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', 'component_id' => $input['this_component_id']));
		// $test = $vce->get_datalist(array('datalist' => 'resource_library_taxonomy'));

		if (isset($test_for_taxonomy_datalist) && $test_for_taxonomy_datalist == NULL) {
			$attributes = array (
				'datalist' => 'pbc_resource_library_taxonomy',
				'aspects' => array ('name' => $input['this_url'] . ' Pbc Resource Library Taxonomy'),
				'component_id' => $input['this_component_id']
			);
			$vce->create_datalist($attributes);
		} elseif (!is_bool(current($test_for_taxonomy_datalist)) && current($test_for_taxonomy_datalist)['datalist'] == 'pbc_resource_library_taxonomy') {
		...
		$content = '';	
		// get datalist that is associated with this component
		$taxonomy = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', 'component_id' => $input['this_component_id']));
...
EOF;
	...
	$vce->page->display_components($resource);
		...
		// get datalist that is associated with this component
		$taxonomy = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', $vce->assigned_taxonomy_component_id));
		...
		$resource_keywords_input = $vce->content->create_input($input,'Resource Keywords');
		...
		// dossier for update
		$this_query_string = NULL;
		if (isset($vce->query_string)) {
			
			$query_string_array = array();
			foreach ($vce->query_string as $k=>$v) {
				$query_string_array[] = $k . '=' . $v;
			}
			$this_query_string = implode('&', $query_string_array);
			$this_query_string = '?' . $this_query_string;

		}
		$this_page = $vce->site->site_url . '/' . $vce->requested_url . $this_query_string;
// $vce->dump($this_page );
// $vce->dump($this_page );
			$dossier = array(
			'type' => 'Pbc_resource_library',
			...
			$resource_keywords_input

EOF;
 			// $resource_category_input  (originially right under $resource_title_input)
		...
		if ($vce->query_string->mode == 'edit') {
			...
			$content .= self::taxonomy_menus($requested_id, $vce->assigned_taxonomy_component_id);
...
EOF;
			...
			$dossier = array(
				...
				'created_at' => $requested_id['created_at'],
				'media_type' => $requested_id['media_type'],
				);
...
EOF;
	...
	private function manage_taxonomy($each_component, $vce) {
		...
		// $vce->dump($each_component);
		// get datalist that is associated with this component
		$taxonomy = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy','component_id' =>  $each_component->component_id));
		...
		if (empty($taxonomy)) {
			...
			$vce->content->add('main','<div class="form-message form-error">An error occured when attempting to find resource_library_taxonomy datalist. <br> Adding a taxonomy for this instance now.</div>');

			// create native taxonomy if it doesn't already exist
			$attributes = array (
				'datalist' => 'pbc_resource_library_taxonomy',
				'aspects' => array ('name' => $each_component->url . ' Pbc Resource Library Taxonomy'),
				'component_id' => $each_component->component_id
...
EOF;
		...
		if (isset($taxonomy['items'])) {
			foreach ($taxonomy['items'] as $item_id=>$item_info) {
				...
				$category_name = $item_info['category_name'].'; item_id: '.$item_id;
...
EOF;
	...
	public function build_taxonomy_custom($taxonomy, $content = null, $counter = 0) {
		...
		if ($taxonomy == 'unqueried') {
				// get datalist that is associated with this component
				$taxonomy1 = $vce->get_datalist(array('datalist' => 'pbc_resource_library_taxonomy', 'component_id' => $vce->assigned_taxonomy_component_id));
...
EOF;
			...
			// build taxonomy menus
			$content .= self::taxonomy_menus($requested_id, $each_component->component_id);
...
EOF;
	...
	public function update_resource($input) {
		...
		$component_id = $input['resource_id'];
		$reload_url = $input['reload_url'];
		unset($input['type'],$input['component_id'],$input['reload_url']);
	...
	public function search_resources($input) {
		...
		// $query = "SELECT component_id, meta_key, meta_value FROM " . TABLE_PREFIX . "components_meta WHERE meta_key IN ('title','description','media_type','keywords') AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components_meta WHERE meta_key IN ('title','description','keywords') AND meta_value LIKE '%" . $search_term . "%' AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components_meta WHERE meta_key='taxonomy') AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components_meta WHERE meta_key='resource_library_component_id' AND meta_value='" . $input['resource_library_component_id'] . "'))";
		$query = "SELECT component_id, meta_key, meta_value FROM " . TABLE_PREFIX . "components_meta WHERE meta_key IN ('title','description','media_type','keywords') AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components_meta WHERE meta_key IN ('title','description','keywords') AND meta_value LIKE '%" . $search_term . "%' AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components WHERE parent_id=$resource_library_component_id) AND component_id IN (SELECT component_id FROM " . TABLE_PREFIX . "components_meta WHERE meta_key='resource_library_component_id' AND meta_value='" . $input['resource_library_component_id'] . "'))";
	// $vce->log($query);	
...
$vce->log($vce->search_results);
        echo json_encode(array('response' => 'success', 'message' => 'Searching for "' . $input['search'] . '"... ',   'procedure' => 'search', 'action' => 'reload', 'url' => $input['this_url']));
        return;
	...
	public function recipe_fields($recipe) {
		...
		$assigned_taxonomy = isset($recipe['assigned_taxonomy']) ? $recipe['assigned_taxonomy'] : null;
...
EOF;
	...
	//assigned taxonomy input
	$input = array(
		'type' => 'select',
		'name' => 'assigned_taxonomy',
		'class' => 'assigned-taxonomy',
		'value' => '',
		'options' => array(
			0 => array(
				'value' => '',
	...
	$query = "SELECT a.meta_value as name, a.datalist_id, c.component_id FROM " . TABLE_PREFIX . "datalists_meta as a join " . TABLE_PREFIX . "datalists_meta as b on a.datalist_id=b.datalist_id and a.meta_key='name' and b.meta_key='datalist' and b.meta_value= 'pbc_resource_library_taxonomy' JOIN " . TABLE_PREFIX . "datalists AS c on a.datalist_id=c.datalist_id";
	$results = $vce->db->get_data_object($query);
	
	if (!empty($results)) {
		$i = 1;
		foreach ($results as $this_data) {
	
			$selected = false;
			if ($this_data->component_id == $assigned_taxonomy) {
				$selected = true;
			}
			$input['options'][$i] = array(
				'value' => $this_data->component_id,
				'name' => $this_data->name,
				'selected' => $selected
			);
			$i++;
	...
	$elements .= $vce->content->create_input($input,'Assigned Taxonomy');
		return $elements;
