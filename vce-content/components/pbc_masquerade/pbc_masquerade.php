...
class Pbc_masquerade extends Component {
	...
	public function component_info() {
		return array(
			'name' => 'PBC Masquerade',
			'description' => 'This is for SANDBOX users to masquerade as specific sandbox roles and test-users.',
			'category' => 'pbc'
		);
	}
        ...
        public function as_content($each_component, $page) {
                ...
                if (isset($sandbox_id['organization'])) {
					$page->filter_by_organization = $sandbox_id['organization'];
				}
...
//                 global $site;
// 				$page->user_search_results = self::sandbox_search();
// 				$site->dump($page->user_search_results);
			...
			$all_users[$value['user_id']][$value['meta_key']] = user::decryption($value['meta_value'], $all_users[$value['user_id']]['vector']);
                        ...
                        foreach ($all_users as $each_user=>$value) {
                                ...
                                foreach ($value as $each_metadata=>$value2) {
                                        // add the values into the user object
                                        $user_object[$each_metadata] = $db->clean($value2);
                                }
...
EOF;
	 ...
	 *
	 */
	public function as_contentBAK($each_component, $page) {
