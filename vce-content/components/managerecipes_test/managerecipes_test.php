...
class ManageRecipes_test extends Component {
    ...
    public function component_info() {
        return array(
            'name' => 'Manage Recipes Test',
            'description' => 'To track down sessions bug',
            'category' => 'test',
    ...
    public function as_content($each_component, $vce) {
        $vce->dump('Here is the PHP $_SESSION: ');
        $vce->dump($_SESSION);
        $vce->dump('Here is the $vce->component_id: ');
        $vce->dump($vce->component_id);
...
EOF;
            ...
            $accordion .= $this->cycle_though_recipe($vce, $recipe);

            // create dossier, which is an encrypted json object of details uses in the form
            $dossier = $vce->generate_dossier(array('type' => 'ManageRecipes_test', 'procedure' => 'update'));
...
EOF;
            ...
            // create dossier, which is an encrypted json object of details uses in the form
            $dossier = $vce->generate_dossier(array('type' => 'ManageRecipes_test', 'procedure' => 'create'));
...
EOF;
            ...
            if (isset($each_item['components'])) {
				...
				$content .= $this->cycle_though_recipe($vce, $each_item['components'], $parent_id, $parent_exists);
    ...
    public function create($input) {
        echo json_encode(array('response' => 'success', 'message' => 'Recipe Created disabled'));
        return;
        ...
        $this->process_recipe($recipe_array, $recipe);
    ...
    public function update($input) {
        ...
        echo json_encode(array('response' => 'updated', 'message' => 'Update disabled'));
        return;
    ...
    private function process_recipe($recipe, $recipe_object, $previous = '0', $level = 1) {
        ...
        foreach ($recipe as $order => $each_recipe) {
            ...
            if ($sub_components) {
                ...
                $this->process_recipe($sub_components, $recipe_object, $component_id, $level);
    ...
    public function delete($input) {
        echo json_encode(array('response' => 'success', 'message' => 'delete disabled', 'form' => 'delete'));
        return;
    ...
    public function update_json($input) {
        ...
        echo json_encode(array('response' => 'success', 'message' => 'JSON alter disabled', 'form' => 'update_json'));
        return;
