...
class Nestor_reports extends Component {
	...
	public function component_info() {
		return array(
			'name' => 'Nestor Reports',
			'description' => 'Development version of a universal Nestor reporting tool',
	...
	/**
	 * loads the <...>Type parent class before the children classes are loaded
	 */
	public static function require_once_nestor_reportstype($vce) {
		if (!class_exists('Nestor_reportsType')) {
...
public function as_content($each_component, $vce) {
	...
	$dossier_for_js_to_db = $vce->generate_dossier($dossier);

	$content .= <<<EOF

	<span id="js_to_db_info" url="$input_path" dossier="$dossier_for_js_to_db" ></span>
EOF;

	// $vce->dump($vce->site->site_roles);
...
$input = array(
	...
	'name' => 'start_date',
	'required' => 'true',
	'class' => 'datepicker refresh-dynamic main-input-table-cell',
	'id' => "start-date-main",
	'data' => array (
		'autocapitalize' => 'none',
		'tag' => 'required',
...
$form_start_date = $vce->content->input_element($input);
...
$form_end_date = $vce->content->input_element($input);

$form_start_date = <<<EOF
<div class="input-label-style main-table-cell"> 
<div class="input-label-text"><label class="input-label-message" for="start-date-main">Start Date</label><div class="input-label-error" role="alert">Start Date is required</div></div>
$form_start_date
</div>
EOF;
...
$form_end_date = <<<EOF
<div class="input-label-style main-table-cell"> 
<div class="input-label-text"><label class="input-label-message" for="end-date-main">End Date</label><div class="input-label-error" role="alert">End Date is required</div></div>
$form_end_date
</div>
EOF;
$dossier_for_summary = $vce->user->encryption(json_encode(array('type' => 'Nestor_reports','procedure' => 'create_summary_table',  'user_id' => $vce->user->user_id, 'component_id' => $each_component->component_id, 'datalist' => 'summary_preset', 'datalist_id'=>$summary_preset_id, 'created_at' => $each_component->created_at, 'url' => $each_component->url)),$vce->user->session_vector);
		...
		$content .= <<<EOF
		<div class="main-datepickers">
		<form class="asynchronous-form background-form main-date-form" method="post" action="$vce->input_path">
		<input type="hidden" name="dossier" value="$dossier_for_summary">
		<div class="main-input-table">
		$form_start_date
		$form_end_date
		<button id="refresh-main-date-button" class="button__primary" type="submit">Resubmit</button>
		</div>
		</form>
		</div>
EOF;
...
$report_results_accordion_content = <<<EOF
	<div id="report-results-accordion-container" class="accordion-container hidden-on-load"> $table_contents </div>
EOF;

		$summary_table_content .= <<<EOF

			$report_results_accordion_content
...
EOF;
				...
				$content .= $vce->content->accordion('Report Results', $summary_table_content, $accordion_expanded = TRUE, $accordion_disabled = false, $accordion_class = 'coaching-companion-summary');
...
$form_end_date = $vce->content->create_input($input,'End Date');
		...
		$form_start_date
		$form_end_date
		<button id="refresh-summary-date-button" class="button__primary" type="submit">Refresh Date Range</button>
		</div>
		</form>
...
EOF;
			...
			if ((isset($each_component->show_analytics_report) && $each_component->show_analytics_report == 'on') || (isset($this->configuration['show_analytics_report']) && $this->configuration['show_analytics_report'] == 'on'))  {
				...
				$analytics_report_content = $this->analytics_report_content($analytics_report_content_input, $each_component, $last_analytics_preset_id);
...
EOF;
		...
		public function data_report_content($data_report_content_input = array(), $each_component, $last_personal_preset_id) {
...
EOF;
			...
			<div id="data-message" style="display:none; background-color: #9DF2A3;">This table has been copied to the clipboard.</div>
				...
				<table id="data-table2" class="tablesorter" border=1> 
					<thead id="data-table-head2"> 
					</thead> 
...
EOF;
		...
		public function analytics_report_content($data_report_content_input = array(), $each_component, $last_analytics_preset_id) {
...
public function get_component_data($vce, $current_page, $page_size, $component_start_date, $component_end_date, $show_all_keys = 'off', $order_by_list) {
...
public function get_analytics_data($vce, $current_page, $page_size, $analytics_start_date, $analytics_end_date, $show_all_keys = 'off', $order_by_list) {
	// $vce->log($analytics_start_date);
	// exit;
	...
	$one_page_ahead = $paginated_component_start_date + $page_size;
	$paginated_component_end_date = ($one_page_ahead <= $analytics_end_date) ? $paginated_component_start_date + $page_size : $component_end_date;
...
/*
	Let javascript communicate with the DB using a dossier and other info saved in a span on the page
*/
public function js_to_db($input) {
	global $vce;
	if (isset($input['views'])) {
		$state_last = $input['views'];
		$vce->log($state_last);
	
		echo json_encode(array('response' => 'success','procedure' => 'js_to_db','action' => 'reload','message' => "saved state"));
		return;
	}
	
	if (isset($input['get_state'])) {
		$state_last = "eyJjdXJyZW50X3ZpZXciOiJzdW1tYXJ5IiwiZGF0YV92aWV3cyI6eyJzdW1tYXJ5Ijp7InRoaXN0aGluZyI6eyJ0aGF0dGhpbmciOiJvbmUifX0sImNvbXBvbmVudF9kYXRhIjp7fSwiYW5hbHl0aWNzIjp7fX19";
		$vce->log($state_last);
		echo json_encode(array('response' => 'success','procedure' => 'js_to_db','action' => 'reload', 'state'=>$state_last, 'message' => "retrieved state"));
		return;
	}

	if (isset($input['test_scope'])) {
		$state_last = "test scope output";
		$vce->log($state_last);
		echo json_encode(array('response' => 'success','procedure' => 'js_to_db','action' => 'reload', 'state'=>$state_last, 'message' => "retrieved state"));
		return;
	}
