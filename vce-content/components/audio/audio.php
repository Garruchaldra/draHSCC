...
class Audio extends MediaType {
	...
	public function display($each_component, $vce) {
    	...
    	// <video controls="" autoplay="" name="media"><source src="" type="audio/mpeg"></video>
     	...
     	$vce->content->add('main', '<audio controls><source src="' . $vce->site->media_link($fileinfo) . '" type="audio/x-m4a">Your browser does not support the audio element.</audio>');
    	
    	// $vce->content->add('main', '<video controls="" autoplay="" name="media"><source src="' . $vce->site->media_link($fileinfo) . '" type="audio/x-m4a"></video>');
