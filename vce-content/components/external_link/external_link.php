...
class ExternalLink extends MediaType {
	...
	public function display($each_component, $vce) {
		...
		if ($http_headers && (strpos($http_headers[0], '200') || strpos($http_headers[0], '303'))) { 
			...
			$contents .= '<p><div class="download-button"><a class="link-button download-button-externallink" href="' . $each_component->link . '" target="_blank"><p class="download-text">' . $each_component->title . '<br>' . $each_component->link . '</p>Click here to open website</a></div></p>';
...
EOF;
		...
		$create = ExternalLink::language('Create');
...
EOF;
		...
		return '<div id="text-block-container add-container">' . $vce->content->accordion(ExternalLink::language('Add An External Link'), $accordion) . '</div>';
