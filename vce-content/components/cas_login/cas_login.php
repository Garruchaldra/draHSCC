...
// 				}
	...
	// $vce->dump('email user_id');
	// $vce->dump($user_id);
					// we have found that the user is still using the email as their lookup, so we need to change it to the CAS UID
					if (!empty($user_id)) {
	...
	// exit;
				...
				// we have checked either the UID or the email and found the user
				if (!empty($user_id)) {

					// check to see if a UID user attribute has been saved to the DB
